...
public class AttributeType {
    ...
    public static AttributeType SERIALIZATION_MODE_TYPE = new AttributeType(new EnumValidator<ImageSerializationMode>("SerializationMode",
                                                                                                                      ImageSerializationMode.values()));

    public static AttributeType IMAGE_SELECTION_MODE_TYPE = new AttributeType(new EnumValidator<ImageSelectionMode>("ImageSelectionMode",
                                                                                                                    ImageSelectionMode.values()));

    public static AttributeType DASH_ARRAY_TYPE = new AttributeType(new ArrayValidator(NumberValidator.INSTANCE));

    public static AttributeType LINE_CAP_TYPE = new AttributeType(new EnumValidator<LineCap>("LineCap",
                                                                                             LineCap.values()));

    public static AttributeType LINE_JOIN_TYPE = new AttributeType(new EnumValidator<LineJoin>("LineJoin",
                                                                                               LineJoin.values()));

    public static AttributeType DRAG_BOUNDS_TYPE = new AttributeType(DragBoundsValidator.INSTANCE);

    public static AttributeType DRAG_CONSTRAINT_TYPE = new AttributeType(new EnumValidator<DragConstraint>("DragConstraint",
                                                                                                           DragConstraint.values()));

    public static AttributeType DRAG_MODE_TYPE = new AttributeType(new EnumValidator<DragMode>("DragMode",
                                                                                               DragMode.values()));

    public static AttributeType TEXT_ALIGN_TYPE = new AttributeType(new EnumValidator<TextAlign>("TextAlign",
                                                                                                 TextAlign.values()));

    public static AttributeType TEXT_BASELINE_TYPE = new AttributeType(new EnumValidator<TextBaseLine>("TextBaseLine",
                                                                                                       TextBaseLine.values()));

    public static AttributeType TEXT_UNIT_TYPE = new AttributeType(new EnumValidator<TextUnit>("TextUnit",
                                                                                               TextUnit.values()));

    public static AttributeType COMPOSITE_OPERATION_TYPE = new AttributeType(new EnumValidator<CompositeOperation>("CompositeOperation",
                                                                                                                   CompositeOperation.values()));

    public static AttributeType ARROW_TYPE = new AttributeType(new EnumValidator<ArrowType>("ArrowType",
                                                                                            ArrowType.values()));

    public static AttributeType FILL_TYPE = new MultiAttributeType("Color or Gradient",
                                                                   COLOR_TYPE,
                                                                   LINEAR_GRADIENT_TYPE,
                                                                   PATTERN_GRADIENT_TYPE,
                                                                   RADIAL_GRADIENT_TYPE);
    ...
    public static AttributeType DIRECTION_TYPE = new AttributeType(new EnumValidator<Direction>("Direction",
                                                                                                Direction.values()));

    public static AttributeType EVENT_PROPAGATION_MODE_TYPE = new AttributeType(new EnumValidator<EventPropagationMode>("EventPropagationMode",
                                                                                                                        EventPropagationMode.values()));
    ...
    public void validate(final JSONValue val,
                         final ValidationContext ctx) throws ValidationException {
        m_validator.validate(val,
                             ctx);
    }

    public static class MultiAttributeType extends AttributeType {

        public MultiAttributeType(final String desc,
                                  final AttributeType... types) {
            super(new MultiTypeValidator(desc,
                                         types));
        }
    }
}
