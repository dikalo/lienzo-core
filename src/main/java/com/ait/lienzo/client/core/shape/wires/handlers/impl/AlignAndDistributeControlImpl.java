...
public class AlignAndDistributeControlImpl implements AlignAndDistributeControl {
    ...
    public AlignAndDistributeControlImpl(IPrimitive<?> group,
        ...
        final Point2D absLoc = group.getComputedLocation();
        ...
        captureHorizontalPositions(left,
                                   right);
        captureVerticalPositions(top,
                                 bottom);
        ...
        m_tranOp = any(Attribute.ROTATION,
                       Attribute.SCALE,
                       Attribute.SHEAR);
    }
    ...
    private final AttributesChangedHandler ShapeAttributesChangedHandler = new AttributesChangedHandler() {
        ...
        public void onAttributesChanged(AttributesChangedEvent event) {
            refresh(event.evaluate(m_tranOp),
                    event.evaluate(m_bboxOp));
        }
    };

    public void addHandlers(IDrawable<?> drawable,
                            ArrayList<Attribute> list) {
        for (Attribute attribute : list) {
            m_attrHandlerRegs.register(drawable.addAttributesChangedHandler(attribute,
                                                                            ShapeAttributesChangedHandler));
        }
        m_attrHandlerRegs.register(drawable.addAttributesChangedHandler(Attribute.ROTATION,
                                                                        ShapeAttributesChangedHandler));
        m_attrHandlerRegs.register(drawable.addAttributesChangedHandler(Attribute.SCALE,
                                                                        ShapeAttributesChangedHandler));
        m_attrHandlerRegs.register(drawable.addAttributesChangedHandler(Attribute.SHEAR,
                                                                        ShapeAttributesChangedHandler));
    }
    ...
    public void capturePositions(double left,
                                 double right,
                                 double top,
                                 double bottom) {
    ...
    public void captureHorizontalPositions(double left,
                                           double right) {
    ...
    public void captureVerticalPositions(double top,
                                         double bottom) {
    ...
    public void updateIndex() {
        ...
        final Point2D absLoc = m_group.getComputedLocation();
    ...
    public void updateIndex(boolean leftChanged,
        ...
        if (leftChanged || rightChanged) {
            ...
            if (leftChanged) {
                m_alignAndDistribute.removeLeftAlignIndexEntry(this,
                                                               m_left);
            }

            if (hCenterChanged) {
                m_alignAndDistribute.removeHCenterAlignIndexEntry(this,
                                                                  m_hCenter);
            }

            if (rightChanged) {
                m_alignAndDistribute.removeRightAlignIndexEntry(this,
                                                                m_right);
            }
            ...
            captureHorizontalPositions(left,
                                       right);
            if (leftChanged) {
                m_alignAndDistribute.addLeftAlignIndexEntry(this,
                                                            m_left);
            }

            if (hCenterChanged) {
                m_alignAndDistribute.addHCenterAlignIndexEntry(this,
                                                               m_hCenter);
            }

            if (rightChanged) {
                m_alignAndDistribute.addRightAlignIndexEntry(this,
                                                             m_right);
            }
        ...
        if (topChanged || bottomChanged) {
            ...
            if (topChanged) {
                m_alignAndDistribute.removeTopAlignIndexEntry(this,
                                                              m_top);
            }

            if (vCenterChanged) {
                m_alignAndDistribute.removeVCenterAlignIndexEntry(this,
                                                                  m_vCenter);
            }

            if (bottomChanged) {
                m_alignAndDistribute.removeBottomAlignIndexEntry(this,
                                                                 m_bottom);
            }
            ...
            captureVerticalPositions(top,
                                     bottom);
            if (topChanged) {
                m_alignAndDistribute.addTopAlignIndexEntry(this,
                                                           m_top);
            }

            if (vCenterChanged) {
                m_alignAndDistribute.addVCenterAlignIndexEntry(this,
                                                               m_vCenter);
            }

            if (bottomChanged) {
                m_alignAndDistribute.addBottomAlignIndexEntry(this,
                                                              m_bottom);
            }
    ...
    @Override
    public void refresh(boolean transforms,
                        boolean attributes) {
    ...
    public void removeChildrenIfIndexed(IPrimitive<?> prim,
                                        List<ShapePair> pairs) {
        for (IPrimitive<?> child : prim.asGroup().getChildNodes()) {
            AlignAndDistributeControl handler = m_alignAndDistribute.getControlForShape(child.uuid());
            if (handler != null) {
                ShapePair pair = new ShapePair(prim.asGroup(),
                                               child,
                                               handler);
                pairs.add(pair);
            ...
            if (child instanceof Group) {
                removeChildrenIfIndexed(child.asGroup(),
                                        pairs);
            }
        }
    }
    ...
    public boolean dragAdjust(Point2D dxy) {
        ...
        double width = m_box.getWidth();
        double height = m_box.getHeight();
        capturePositions(left,
                         left + width,
                         top,
                         top + height);

        AlignAndDistribute.AlignAndDistributeMatches matches = m_alignAndDistribute.findNearestMatches(this,
                                                                                                       m_left,
                                                                                                       m_hCenter,
                                                                                                       m_right,
                                                                                                       m_top,
    ...
    public void dragEnd() {
        ...
        removeChildrenIfIndexed(m_group,
                                pairs);
