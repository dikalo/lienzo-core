...
public class MagnetManager {
    ...
    public ImageData drawMagnetsToBack(Magnets magnets,
                                       NFastStringMap<WiresShape> shapeColors,
                                       NFastStringMap<WiresMagnet> magnetColors,
                                       ScratchPad scratch) {
        scratch.clear();
        Context2D ctx = scratch.getContext();

        drawShapeToBacking(magnets,
                           shapeColors,
                           ctx);

        magnetColors.clear();
        for (int i = 0; i < magnets.size(); i++) {
            drawMagnet(magnetColors,
                       ctx,
                       magnets.getMagnet(i));
        }
        return ctx.getImageData(0,
                                0,
                                scratch.getWidth(),
                                scratch.getHeight());
    }

    protected void drawShapeToBacking(Magnets magnets,
                                      NFastStringMap<WiresShape> shapeColorMap,
                                      Context2D ctx) {
        // the Shape doesn't need recording, we just need to know the mouse is over something
        BackingColorMapUtils.drawShapeToBacking(ctx,
                                                magnets.getWiresShape(),
                                                m_c_rotor.next(),
                                                shapeColorMap);
    }

    protected void drawMagnet(NFastStringMap<WiresMagnet> magnetColorMap,
                              Context2D ctx,
                              WiresMagnet m) {
        String c = m_c_rotor.next();
        magnetColorMap.put(c,
                           m);
        ctx.beginPath();
        ctx.setStrokeWidth(m_ctrlSize);
        ctx.setStrokeColor(c);
        ctx.setFillColor(c);
        ctx.arc(m.getControl().getX(),
                m.getControl().getY(),
                m_ctrlSize,
                0,
                2 * Math.PI,
    ...
    public Magnets createMagnets(final WiresShape wiresShape) {
        ...
        final Magnets magnets = new Magnets(this,
                                            list,
                                            wiresShape);
        ...
        for (Point2D p : points) {
            ...
            WiresMagnet m = new WiresMagnet(magnets,
                                            null,
                                            0,
                                            p.getX(),
                                            p.getY(),
                                            getControlPrimitive(mx,
                                                                my),
                                            true);
    ...
    private Circle getControlPrimitive(double x,
                                       double y) {
    ...
    public static class Magnets implements AttributesChangedHandler,
        ...
        public Magnets(MagnetManager magnetManager,
                       IControlHandleList list,
            ...
            m_registrationManager.register(shapeGroup.addAttributesChangedHandler(Attribute.X,
                                                                                  this));
            m_registrationManager.register(shapeGroup.addAttributesChangedHandler(Attribute.Y,
                                                                                  this));
        ...
        public void onAttributesChanged(AttributesChangedEvent event) {
            if (!m_isDragging && event.any(Attribute.X,
                                           Attribute.Y)) {
                shapeMoved();
            }
        }
        ...
        public void shapeMoved() {
            final IPrimitive<?> prim = getPrimTarget();
